...
### 3. Structure code with objects, in Object Oriented Programming fashion (OOP)
...
- Formalise file reading as a backend, accepting target file(s)
  - it should yield data points for the process to handle
- Formalise model as a process, accepting backend object and process function
  - for each of the data points yielded from the backend, it process it using the processing function
- Rewrite process function to conform to OOP structure

**TIP:** if you notice some inefficiencies arising from the above specification, then please feel free to implement your solution differently, whilst motivating why.
